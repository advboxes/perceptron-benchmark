...
def anchor_targets_bbox(
    ...
    ----------
    anchors : array
        np.array of annotations of shape (N, 4) for (x1, y1, x2, y2).
    image_group : list
        List of BGR images.
    annotations_group : list
        List of annotations (np.array of shape (N, 5) for (x1, y1, x2, 
        y2, label)).
    num_classes : int
        Number of classes to predict.
    negative_overlap : float
        IoU overlap for negative anchors (all anchors with overlap < 
        negative_overlap are negative).
    positive_overlap : float
        IoU overlap or positive anchors (all anchors with overlap > 
        positive_overlap are positive).

    Returns
        labels_batch: batch that contains labels & anchor states (np.array 
        of shape (batch_size, N, num_classes + 1),
        where N is the number of anchors for an image and the last column 
    ...
    for index, (image, annotations) in enumerate(
        ...
        if annotations['bboxes'].shape[0]:
            ...
            positive_indices, ignore_indices, argmax_overlaps_inds = \
                ...
                anchors, annotations['bboxes'], negative_overlap, 
                positive_overlap)
        ...
        if image.shape:
            anchors_centers = np.vstack(
                [(anchors[:, 0] + anchors[:, 2]) / 2, (anchors[:, 1] + \
                    anchors[:, 3]) / 2]).T
...
def layer_shapes(image_shape, model):
    ...
    model : keras model
        The model to use for computing how the image shape is 
...
def anchors_for_shape(
    ...
    image_shape : list
        The shape of the image.
    pyramid_levels : list
        List of ints representing which pyramids to use 
        (defaults to [3, 4, 5, 6, 7]).
    anchor_params : Struct containing anchor parameters. 
        If None, default values are used.
    shapes_callback : function
        Function to call for getting the shape of the image at different 
        pyramid levels.

    Returns
        np.array of shape (N, 4) containing the (x1, y1, x2, y2) 
...
def bbox_transform(anchors, gt_boxes, mean=None, std=None):
    ...
    if isinstance(mean, (list, tuple)):
        ...
        raise ValueError(
            'Expected mean to be a np.ndarray, list or tuple. Received: '\
                '{}'.format(
                type(mean)))
    ...
    if isinstance(std, (list, tuple)):
        ...
        raise ValueError(
            'Expected std to be a np.ndarray, list or tuple. Received: '\
                '{}'.format(
                type(std)))
